...
def test_offset_regular(page: Page, browser: Browser, browser_type: BrowserType):
    ...
    price_regular = step_one.priceSelector.get_price_second()
    price_urgent = step_one.priceSelector.get_price_first()
    ready_days_urgent, ready_date_urgent = step_one.priceSelector.get_ready_time_first()
    ready_days_regular, ready_date_regular = step_one.priceSelector.get_ready_time_second()

    assert (price_urgent == 1302)
    assert (price_regular == 1260)

    assert (ready_days_regular == '2')
    assert (ready_days_urgent == '1')

    assert (ready_date_regular == '29 августа')
